...
export class UsersService {
  ...
  async facebookRegister(facebookUserDto: {
    email: string;
    name: string;
  }): Promise<any> {
    if (!facebookUserDto.email || !facebookUserDto.name) {
      throw new NotFoundException('Preencha todos os campos');
    }
    ...
    try {
      ...
      throw new Error(
        `Erro ao registrar usuário via Facebook: ${error.message}`,
      );
    }
  }
